...
namespace CSharpKOTOR.Common
{
    ...
    public class LocalizedString : IEquatable<LocalizedString>, IEnumerable<(Language, Gender, string)>
    {
        ...
        private readonly Dictionary<int, string> _substrings = new Dictionary<int, string>();

        public LocalizedString(int stringRef, [CanBeNull] Dictionary<int, string> substrings = null)
        {
            StringRef = stringRef;
            if (substrings != null)
            {
        ...
        public static LocalizedString FromInvalid() => new LocalizedString(-1);
